...
func spread_stat(ev: GameEvent) -> void:
	...
	# Source
	var before_src: float = float(cult_data.stats_matrix[src][stat_idx])
	var after_src: float = clamp(before_src + strength, 0.0, 100.0)
	cult_data.stats_matrix[src][stat_idx] = after_src
	if src < cult_data.npcs.size():
	...
	for tgt in range(count):
		...
		var before: float = float(cult_data.stats_matrix[tgt][stat_idx])
		var after: float = clamp(before + delta, 0.0, 100.0)
		cult_data.stats_matrix[tgt][stat_idx] = after
		if tgt < cult_data.npcs.size():
...
func _apply_stat_delta(npc_id: int, stat_name: String, delta: float) -> void:
	...
	if idx == -1: return
	cult_data.stats_matrix[npc_id][idx] += delta
...
func _build_social_clusters() -> void:
	...
	for i in n:
		...
		for j in n:
			...
			if cult_data.relationship_matrix[i][j] > 70.0 and cult_data.relationship_matrix[j][i] > 70.0:
				cluster.append(j)
				assigned[j] = 1
		if cluster.size() > 1:
